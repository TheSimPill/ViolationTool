...
def scrape_fines(reparse, states, dir):
    # Create a new directory for all save data
    dir = dir + "/webscraper_data"
    os.mkdir(path=dir)


    session = random.randint(900, 793293)
    params = {
    "api_key":"wXum603Fr6BWLKe41ezIibloX4yAkrLi",
    ...
    for state_url in rows:
        ...
        state_home_links = get_state_fine_links(reparse, params, state)
...
def scrape_facility(args):
    ...
    while True:
        ...
        if (not reparse and not exists(dir + "/" + state + str(facilitynumber) + "-page.html")) or reparse or (retries >= 5):
